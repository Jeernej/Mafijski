...
#delta=1.1
#ksi1=0.40102935967035874
#ksi2=2.6578151164488752
...
def F(pribl,N,delta):
    ...
    for j in range(1,N-1,1): # iteracijo začnemo v točkah ob robnih/začetnih pogojih; (torej od druge do predzadnje)
        ...
        F[j]=-0.5*h**2.*(-(pribl[j+1]-2.*pribl[j]+pribl[j-1])/(h**2.)+ddy(pribl[j],delta))
...
def NEWTON(pribl,N,eps,delta):
    ...
    Y=pribl  # ima N+1 točk
    dY=[eps+1 for j in range(N-1)]  # ima eno točko manj (N-1 točk), ker ne računamo v robnih pogojih /beri dalje
...
##_________ Analitični rešitvi KONVERGENČNI PARAMETER
##
#
...
#_________ optimalni KONVERGENČNI PARAMETER
...
#_____________ REŠITVE variacije po w 
...
##
####_____________ NAPAKE variacije po w 
##
#FIGerr= plt.figure()
